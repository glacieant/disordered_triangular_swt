...
def classic_zmc(long CLNUM,long LSYS,double[:,:] J,
        double[:,:,:] M,double TOL):

    # declaring C datatypes for loop variables
    cdef long i, j, k, p
    cdef long ix, jx, kx
    cdef long nix, njx, nkx
    cdef long ixp, jxp, kxp, NN
    cdef long MAX_ITER = CLNUM
    cdef long NSYS = LSYS**3
    ...
    for i in range(0,MAX_ITER):
        ...
        for j in range(0,NSYS):
            ...
            for NN in range(-2,3):
                ...
                nix = ix + NN
                if nix >= 0 and nix < LSYS:
                    ixp = nix
                elif nix >= 0 and nix >= LSYS:
                    ixp = nix - LSYS
                else:
                    ixp = nix + LSYS

                k = ixp + jx*LSYS + kx*(LSYS**2)
                ...
                njx = jx + NN
                if njx >= 0 and njx < LSYS:
                    jxp = njx
                elif njx >= 0 and njx >= LSYS:
                    jxp = njx - LSYS
                else:
                    jxp = njx + LSYS

                k = ix + jxp*LSYS + kx*(LSYS**2)
                ...
                nkx = kx + NN
                if nkx >= 0 and nkx < LSYS:
                    kxp = nkx
                elif nkx >= 0 and nkx >= LSYS:
                    kxp = nkx - LSYS
                else:
                    kxp = nkx + LSYS

                k = ix + jx*LSYS + kxp*(LSYS**2)
