...
import os
os.chdir("../ALPHA_DELTA_PHASE/no-angcorr-swt-job1/DATA/")
import numpy as np
...
#loading data
l, delta, alpx, vectchir, vectchir_var = np.loadtxt('LISTDATA.dat', usecols=(0,1,26,16,17), unpack=True)
...
#loading data
vectchircl, vectchircl_var = np.loadtxt('LISTDATA.dat', usecols=(20,21), unpack=True)
...
for d in range(0,DSET):
    ...
    plt.title(r'Unit vector chirality, $\Delta=$'+str("%.3f" % DELTA[d]),fontsize=15, y=1.15)
    #Setting up the tick lines and label formatting
    Y=np.concatenate((vectchir,vectchircl))
    X=np.concatenate((xp,xpc))
    xmin=0.0
    xmax=np.amax(X)*1.1
    ymin=-0.0
    ymax=1.05
    ...
    #plt.show()
    fig.savefig("vectchir_delta="+str("%.3f" % DELTA[d])+".pdf")
