...
for i in range(0,HSHNUM):
    ...
    #YDATA = 1.0/SFX[:(SFX.size/3)]
    YDATA = 1.0/SFX[:4]
    XDATA = np.arange(YDATA.size)**2
    ax.plot(XDATA,YDATA,'b.',label='Data')

    # finding the parabolic fit

    popt, pcov = curve_fit(powe,XDATA,YDATA)
    ...
    PDATA = np.linspace(XDATA.min(),XDATA.max(),
            200)
    ax.plot(PDATA,powe(PDATA, *popt),'k-',
            label=
            r'$\frac{1}{S(\mathbf{q})}\propto'
            +r'|\mathbf{q}-\mathbf{Q}|^b$'
            +r' \ with $b=$'
            +str("%.4f" % b)
            )
