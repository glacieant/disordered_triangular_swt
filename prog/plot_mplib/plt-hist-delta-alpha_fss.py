...
import os
os.chdir("../ALPHA_DELTA_PHASE/wth-angcorr-swt-job1/DATA/")
import numpy as np
...
#loading data
L, delta, alpx, data = np.loadtxt('EIGENDATA.dat', usecols=(0,1,2,7), unpack=True)
...
for l in range(0,LSET):
    ...
    for d in range(0,DSET):
        ...
        for a in range(0,NSET):
            ...
            ax.hist(xp,bins=np.linspace(-0.5,0.5,100))
            plt.xlim(-0.5,0.5)
            plt.title(r'Histogram of local magnetization,L='+str("%d"%LX[l])+',$\Delta=$'
