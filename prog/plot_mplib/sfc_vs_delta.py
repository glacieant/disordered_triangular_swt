...
for AN in range(0,ANUM):
    ...
    for DN in range(0,DNUM):
        ...
        popt, pcov = curve_fit(linear,INVNRAY[0:3],
                SFC_MAX[:,DN,AN][LORD][0:3]
                )

        YDATA = np.abs(SFC_MAX[:,DN,AN][LORD]-popt[0])/INVNRAY
        YDATA_ERR = SFC_MAX_ERR[:,DN,AN][LORD]/INVNRAY
        XDATA = INVLRAY
    ...
    ax.set_ylabel(r'$|S(\vec{Q})_\infty-S(Q)_L|L^2$',fontsize=20)
