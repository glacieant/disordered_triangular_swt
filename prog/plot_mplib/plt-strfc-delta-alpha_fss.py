...
#loading data
l, delta, alpx, strfc, strfc_var, sample_size  = np.loadtxt('LISTDATA.dat', usecols=(0,1,26,24,25,33), unpack=True)
...
for d in range(0,DSET):
    ...
    plt.title(r'Structure factor $\chi(Q)$ at $(4\pi/3,0)$, $\Delta=$'+str("%.3f" % DELTA[d]),fontsize=15, y=1.15)

    #Setting up the tick lines and label formatting
    xmin=0.0
    xmax=np.amax(l)*1.1
    ymin=0.0
    ymax=np.amax(strfc[d*NSET::NSET*DSET])*1.05
    tickfreq=4
    ...
    #plt.show()
    fig.savefig("strfc_delta="+str("%.3f"%DELTA[d])+".pdf")
    plt.close('all')
