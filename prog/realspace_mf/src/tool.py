...
def init_cpl(NSYS,nbr,ZCO,XPAR,DELTA,ALPHA,J):
...
def init_param(NSYS,nbr,ZCO,XPAR,
    ...
    for i in range(0,NSYS):
        ...
        for m in range(0,12):
            ...
            if j > i:
                ...
                #bond[1,i,j] = ((2.0**0.5)*np.random.uniform(-BONDVAR,BONDVAR)
                #        + (2.0**0.5)*np.random.uniform(-BONDVAR,BONDVAR)*1.0j)
                #bond[1,j,i] = bond[1,i,j].conj()
                bond[1,i,j] = 0.4
                bond[1,j,i] = 0.4
    
    M[1] = np.random.uniform(-ANGVAR,ANGVAR,size=(NSYS,3))
    M[1] = 0.5*np.einsum('i,ij->ij',np.einsum('ij,ij->i',M[1],M[1])**(-0.5),M[1])
