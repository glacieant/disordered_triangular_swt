...
def main(const):
    ...
    for i in range(0,ITERDISD):
        ...
        for g in range(0,BOOTNUM): 
            ...
            impsite = (NSYS+1)/2-1
            for p in [3]:
                J[impsite][nbr[impsite][p]] = 0.8
                J[nbr[impsite][p]][impsite] = 0.8
            ...
            # restoring impurity couplings
            
            """
            impsite = (NSYS+1)/2-1
            for p in [3]:
                J[impsite][nbr[impsite][p]] = 1.0
                J[nbr[impsite][p]][impsite] = 1.0
            impsite = (NSYS+1)/2
            for p in [3]:
                J[impsite][nbr[impsite][p]] = 1.0
                J[nbr[impsite][p]][impsite] = 1.0
            ...
            for site in range(0,HSYS):
                for p in [3]:
                    J[site][nbr[site][p]] = 1.0
                    J[nbr[site][p]][site] = 1.0
            for site in range(HSYS,NSYS):
                for p in [1]:
                    J[site][nbr[site][p]] = 1.0
                    J[nbr[site][p]][site] = 1.0
