...
int main(int argc,char *argv[]) {
	...
	//Parameters for classical uniform moment
    ...
    //Parameters for quantum uniform moment
	...
	for (diter=0;diter<iter_DISD;diter++) {
		...
		gen_couplings(N_lattpoint,lsite_tmp[0],
				ALPHA,DELTA,J); // Using the same random hamiltonian
...
#pragma omp for
	...
	int LAT_SMPLE=iter_DISD;

	if (LSTSWITCH != 0) {
		fprintf(outf,"%d\t%lf\t%lf\t%d\t"
                "%lf\t%lf\t%lf\t"
                "%lf\t%lf\t%lf\n",
				L, // 1
				DELTA, // 2
				ALPHA, // 3
				iter_DISD, //4
				SYSAVG(CMOM_X,LAT_SMPLE), // 5
				SYSAVG(CMOM_Y,LAT_SMPLE), // 6
				SYSAVG(CMOM_Z,LAT_SMPLE), // 7
				SYSAVG(QMOM_X,LAT_SMPLE), // 8
				SYSAVG(QMOM_Y,LAT_SMPLE), // 9
				SYSAVG(QMOM_Z,LAT_SMPLE) // 10
				);
