...
void q_moment(int N_lattpoint, lattpoint *lsite, 
	...
	for (i=0;i<N_lattpoint;i++) {
        ...
        VAX=-(sin(th)*cos(ph)*alpha[i][i].re
             -cos(th)*cos(ph)*dthph[i]
             +sin(th)*sin(ph)*dthph[i+N_lattpoint]);
        VAL_X+=VAX;
		VAL_VAR+=VAX*VAX;

        VAX=-(sin(th)*sin(ph)*alpha[i][i].re
             -cos(th)*sin(ph)*dthph[i]
             -sin(th)*cos(ph)*dthph[i+N_lattpoint]);
        VAL_Y+=VAX;
		VAL_VAR+=VAX*VAX;

        VAX=-(cos(th)*alpha[i][i].re
             +sin(th)*dthph[i]);
        VAL_Z+=VAX;
		VAL_VAR+=VAX*VAX;
